...
Reads the binary .gddbg trace format without requiring the Rust differ.
...
def _parse_header(data: bytes) -> TraceHeader:
    ...
    if magic != GDDBG_MAGIC:
        raise ValueError(
            f"Invalid magic: 0x{magic:016x} (expected 0x{GDDBG_MAGIC:016x})"
        )
    if version != GDDBG_VERSION:
        raise ValueError(f"Unsupported version: {version} (expected {GDDBG_VERSION})")
