// HIP/ROCm device-side runtime for PRLX
// Mirrors prlx_runtime.cu but uses HIP intrinsics (wave32 mode)
//
// Key differences from CUDA:
//   - __activemask()   -> __ballot(1)
//   - __shfl_sync()    -> __shfl()
//   - Lane ID via __builtin_amdgcn_mbcnt_lo instead of PTX asm
//   - Cache-bypass PTX st.global.cg -> volatile stores
//   - hip/hip_runtime.h instead of cuda_runtime.h

#include "prlx_runtime.h"
#include <hip/hip_runtime.h>

// Global device pointer to trace buffer (set by host via hipMemcpyToSymbol)
...
// Compute linear warp (wavefront) ID within the grid
...
// Lane ID using AMDGCN intrinsic (wave32 mode)
__device__ __forceinline__ uint32_t __prlx_lane_id() {
    return __builtin_amdgcn_mbcnt_lo(~0u, 0);
}

// Volatile store for 16-byte aligned data (HIP equivalent of cache-bypass PTX store)
// On AMDGPU we use volatile stores to avoid L1 caching pollution
__device__ __forceinline__ void __prlx_store_16b(void* dst, const uint32_t* src) {
    volatile uint32_t* vdst = reinterpret_cast<volatile uint32_t*>(dst);
    vdst[0] = src[0];
    vdst[1] = src[1];
    vdst[2] = src[2];
    vdst[3] = src[3];
...
// Volatile store for a trace event
...
// Claim a trace event slot for the current warp/wavefront.
...
extern "C" __device__ void __prlx_record_branch(
    ...
    evt._reserved = 0;
    evt.active_mask = __ballot(1);  // HIP equivalent of __activemask()
...
extern "C" __device__ void __prlx_record_value(
    ...
    // Track total writes for ordering
...
// Uses __shfl() (HIP) to gather per-lane values to lane 0.
extern "C" __device__ void __prlx_record_snapshot(
    ...
    for (int src = 0; src < 32; src++) {
        uint32_t l = __shfl(lhs, src);  // HIP: no mask parameter needed
        uint32_t r = __shfl(rhs, src);
        if (lane == 0) {
    ...
    // Store using multiple volatile 16-byte writes
    // 288 bytes = 72 x u32 = 18 x 16-byte stores
    const uint32_t* src_ptr = reinterpret_cast<const uint32_t*>(&entry);
    uint32_t* dst_ptr = reinterpret_cast<uint32_t*>(&entries[slot]);
    for (int i = 0; i < 18; i++) {
        __prlx_store_16b(dst_ptr + i * 4, src_ptr + i * 4);
    }
}
