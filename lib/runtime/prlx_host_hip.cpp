// HIP/ROCm host-side runtime for PRLX
// Mirrors prlx_host.cu but uses HIP API calls:
...
extern "C" void prlx_init(void) {
    ...
    if (enabled) {
        fprintf(stderr, "[prlx] Tracing enabled (HIP/ROCm), output: %s\n", output_path);
    }
}
...
extern "C" void prlx_pre_launch(const char* kernel_name, dim3 gridDim, dim3 blockDim) {
    ...
    fprintf(stderr, "[prlx] Allocating trace buffer: %zu MB for %u warps (HIP)\n",
            trace_buffer_size / (1024*1024), total_warps);

    hipError_t err = hipMalloc(&d_trace_buffer, trace_buffer_size);
    if (err != hipSuccess) {
        fprintf(stderr, "[prlx] ERROR: hipMalloc failed: %s\n", hipGetErrorString(err));
        enabled = false;
        return;
    }

    hipMemset(d_trace_buffer, 0, trace_buffer_size);
    ...
    // For AMD GPUs, store the GCN architecture number in cuda_arch field
    // prop.gcnArch gives the GFX ISA version (e.g., 906 for gfx906, 1030 for gfx1030)
    header.cuda_arch = prop.gcnArch;

    hipMemcpy(d_trace_buffer, &header, sizeof(header), hipMemcpyHostToDevice);

    // Set the global device variable g_prlx_buffer to point to this buffer
    err = hipMemcpyToSymbol(HIP_SYMBOL(g_prlx_buffer), &d_trace_buffer, sizeof(void*));
    if (err != hipSuccess) {
        fprintf(stderr, "[prlx] ERROR: hipMemcpyToSymbol failed: %s\n",
                hipGetErrorString(err));
        hipFree(d_trace_buffer);
        d_trace_buffer = nullptr;
        enabled = false;
        return;
    }

    hipMemcpyToSymbol(HIP_SYMBOL(__prlx_sample_rate), &sample_rate, sizeof(uint32_t));
    if (sample_rate > 1) {
    ...
    if (history_depth > 0) {
        ...
        err = hipMalloc(&d_history_buffer, history_buffer_size);
        if (err != hipSuccess) {
            ...
            hipMemset(d_history_buffer, 0, history_buffer_size);
    ...
    if (snapshot_depth > 0) {
        ...
        err = hipMalloc(&d_snapshot_buffer, snapshot_buffer_size);
        if (err != hipSuccess) {
    ...
    fprintf(stderr, "[prlx] Trace buffer ready for kernel: %s (HIP)\n", kernel_name);
}
...
extern "C" void prlx_post_launch(void) {
    ...
    fprintf(stderr, "[prlx] Copying trace buffer from device (HIP)...\n");
    ...
    hipError_t err = hipMemcpy(h_buffer, d_trace_buffer, trace_buffer_size,
                                  hipMemcpyDeviceToHost);
    if (err != hipSuccess) {
        fprintf(stderr, "[prlx] ERROR: hipMemcpy failed: %s\n", hipGetErrorString(err));
        free(h_buffer);
        return;
    }
    ...
    if (d_history_buffer && history_buffer_size > 0) {
        ...
        if (h_history) {
            err = hipMemcpy(h_history, d_history_buffer, history_buffer_size,
                             hipMemcpyDeviceToHost);
            if (err != hipSuccess) {
    ...
    if (d_snapshot_buffer && snapshot_buffer_size > 0) {
        ...
        if (h_snapshot) {
            err = hipMemcpy(h_snapshot, d_snapshot_buffer, snapshot_buffer_size,
                             hipMemcpyDeviceToHost);
            if (err != hipSuccess) {
    ...
    // Build contiguous payload for potential compression
    ...
    if (d_history_buffer) {
        hipFree(d_history_buffer);
        d_history_buffer = nullptr;
        history_buffer_size = 0;

        void* null_ptr = nullptr;
        uint32_t zero = 0;
        hipMemcpyToSymbol(HIP_SYMBOL(g_prlx_history_buffer), &null_ptr, sizeof(void*));
        hipMemcpyToSymbol(HIP_SYMBOL(g_prlx_history_depth), &zero, sizeof(uint32_t));
    }

    if (d_snapshot_buffer) {
        hipFree(d_snapshot_buffer);
        d_snapshot_buffer = nullptr;
        snapshot_buffer_size = 0;
    }

    {
        void* null_ptr = nullptr;
        uint32_t zero = 0;
        hipMemcpyToSymbol(HIP_SYMBOL(g_prlx_snapshot_buffer), &null_ptr, sizeof(void*));
        hipMemcpyToSymbol(HIP_SYMBOL(g_prlx_snapshot_depth), &zero, sizeof(uint32_t));
        hipMemcpyToSymbol(HIP_SYMBOL(g_prlx_buffer), &null_ptr, sizeof(void*));
    }
}
...
extern "C" void prlx_session_begin(const char* name) {
    ...
    fprintf(stderr, "[prlx] Session started (HIP): %s\n", session_dir);
}
